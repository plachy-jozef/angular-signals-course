...
export class CoursesService {
  ...
  async postCourse(course: Partial<Course>): Promise<Course> {
    const response$: Observable<Course> = this.http.post<Course>(`${ this.env.apiRoot }/courses`, course);
    return firstValueFrom(response$);
  }

  async putCourse(courseId: string, course: Partial<Course>): Promise<Course> {
    const response$: Observable<Course> = this.http.put<Course>(`${ this.env.apiRoot }/courses/${ courseId }`, course);
    return firstValueFrom(response$);
  }
